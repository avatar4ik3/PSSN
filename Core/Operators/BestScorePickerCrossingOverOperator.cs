...
public class BestScorePickerCrossingOverOperator
{
    ...
    public BestScorePickerCrossingOverOperator(int crossingSelector, IEnumerable<IStrategy> all, TreeGameRunnerResult results)
    {
    ...
    public IEnumerable<IStrategy> Operate(IStrategy st1, IStrategy st2)
    {
        ...
        foreach (var index in s1Take)
        {
            newS2dic[index] = s1.behaviours[index];
        }
        foreach (var index in s2Take)
        {
